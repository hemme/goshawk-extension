...
let last_options_W = 0, last_options_H = 0
...
function update_title_header() {
    const h1 = Q('h1')
    if (h1) {
        h1.textContent = (param.copy_format === 'hen') ? 'HEN from Image' : 'SGF from Image'
    }
}
...
function read_param(elem, temporary) {
    ...
    draw(0, 0)
    update_title_header()
    const sgf_only_p = elem && elem.name && Object.keys(default_sgf_param).includes(elem.name)
...
Q_all('input, select').forEach(elem => {
...
const [image_ctx, overlay_ctx] = [image_canvas, overlay_canvas].map(c => c.getContext('2d', { willReadFrequently: true }))

function set_size() {
    const left_col = Q('#left_column')
    if (left_col && left_col.clientWidth > 0 && left_col.clientHeight > 0) {
        cw = to_i(left_col.clientWidth)
        ch = to_i(left_col.clientHeight)
    } else {
...
const listener = {mousemove, mousedown, mouseup, contextmenu: e => e.preventDefault()}
...
function draw_fine_tune() {
    ...
    const g = overlay_ctx
    g.strokeStyle = '#c4956a'; g.lineWidth = 1; cross_line(g, ...fine_tune_xy)
}
...
function update_sgf_size_options() {
    const select = Q('#sgf_size_select')
    if (!select) return
    ...
    if (W === last_options_W && H === last_options_H) {
        select.value = param.sgf_size
        return
    }
    ...
    const N = has_grid ? Math.max(W, H) : 0
    const current_value = param.sgf_size
    ...
    add_option('-1', `as is (${W}x${H})`)
    
    const standard_sizes = [9, 13, 19]
    if (has_grid && !standard_sizes.includes(N)) {
        add_option(N.toString(), `${N}x${N}`)
    }
    
    standard_sizes.forEach(size => {
        if (size >= N) {
            add_option(size.toString(), `${size}x${size}`)
        }
    })
    
    let value_exists = Array.from(select.options).some(opt => opt.value === current_value)
    if (value_exists) {
...
function update_alignment_visibility() {
    const select = Q('#sgf_size_select')
    if (!select) return
    
    const size = to_i(param.sgf_size)
    let W = 19, H = 19
    try {
        if (xy_mn && xy_11) {
            const gp = grid_params()
    ...
    const show_h = (size > 0 && W < size)
    const show_v = (size > 0 && H < size)
    
    show_if(show_h, '#align_h_container')
    show_if(show_v, '#align_v_container')
}

function update_indicators() {
    update_sgf_size_options()
    update_alignment_visibility()
    ...
    const highlight_guide = (sel, k) => {
        ...
        style.borderLeft = current ? '3px solid #c4956a' : '3px solid transparent'
        style.color = current ? '#1a1612' : '#6b563d'
    }
...
function update_sgf(silent, temporary) {
    if (electron) {return}
    const sgf = get_sgf()
    const format = param.copy_format || 'sgf'
    
    let output_text = sgf
    if (format === 'hen') {
        try {
            output_text = Hen.sgf2hen(sgf)
        } catch (e) {
            console.error("Failed to generate HEN from SGF:", e)
        }
    }
    ...
    !temporary && navigator.clipboard.writeText(output_text)
    !silent && wink()
}
function update_sgf_silently(temporary) {update_sgf(true, temporary)}

function update_sgf_link(sgf) {
    const btn = Q('#download')
    const datestr = (new Date()).toISOString()
    const filename = datestr.replace(/:|^[-+]|[.].*/g, '').replace('T', '-') + '.sgf'
    btn.dataset.filename = filename
    btn.dataset.content = sgf
    btn.title = 'Download SGF'
}

function update_hen_link(hen) {
    const btn = Q('#download')
    const datestr = (new Date()).toISOString()
    const filename = datestr.replace(/:|^[-+]|[.].*/g, '').replace('T', '-') + '.hen'
    btn.dataset.filename = filename
    btn.dataset.content = hen
    btn.title = 'Download HEN'
}
...
function get_sgf(given_board) {
    ...
    let offset_x = 0
    if (size > W) {
        const align_h = param.align_h || 'center'
        if (align_h === 'center') offset_x = Math.floor((size - W) / 2)
        else if (align_h === 'right') offset_x = size - W
    }
    
    let offset_y = 0
    if (size > H) {
        const align_v = param.align_v || 'center'
        if (align_v === 'center') offset_y = Math.floor((size - H) / 2)
        else if (align_v === 'bottom') offset_y = size - H
    }
    
    const sgfpos_name = "abcdefghijklmnopqrs"
    const header = `(;SZ[${size}]PL[${to_play}]`, footer = ')'
    const sgfpos = (k_aligned, sign) => sgfpos_name[sign > 0 ? k_aligned : size - k_aligned - 1]
    const [sj, si] = coord_signs
    const coords = (i, j) => {
        const j_aligned = j + offset_x
        const i_aligned = i + offset_y
        return '[' + sgfpos(j_aligned, sj) + sgfpos(i_aligned, si) + ']'
    }
    const grids =
